...
import { toggleBlogDropDownSmall, type AppDispatcher, type RootState } from "../store";
// 1. Import LuPlus and LuMinus
import { LuPlus, LuMinus } from 'react-icons/lu';

export default function BlogDropDownSmall() {
  const dispatch: AppDispatcher = useDispatch()
  const { blogDrpDown } = useSelector((state: RootState) => state.slice1)

  return (
    <div
      onClick={() => dispatch(toggleBlogDropDownSmall())}
