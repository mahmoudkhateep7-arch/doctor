...
import { togglePageDropDownSmall, type AppDispatcher, type RootState } from "../store";
import { useDispatch, useSelector } from "react-redux";
import { Link } from "react-router-dom";
import { LuMinus, LuPlus } from "react-icons/lu";
export default function PagesDropDownSmall() {
  const dispatch: AppDispatcher = useDispatch()
  const { pagesDropDown } = useSelector((state: RootState) => { return state.slice1 })
  return (
    <div onClick={() => { dispatch(togglePageDropDownSmall()) }}
