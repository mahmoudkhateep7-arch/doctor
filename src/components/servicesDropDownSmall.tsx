...
import { toggleservicesDropdownSmall, type AppDispatcher, type RootState } from "../store";
import { AnimatePresence } from "motion/react";
import { LuMinus, LuPlus } from 'react-icons/lu';


export default function ServicesDrpDownSmall() {
  const dispatch: AppDispatcher = useDispatch()
  const { servicesDropdown } = useSelector((state: RootState) => { return state.slice1 })
  return (
    <div onClick={() => { dispatch(toggleservicesDropdownSmall()) }}
